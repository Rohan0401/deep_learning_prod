...
class GAN:
    """
    A GAN model, it creates and stores inputs, outputs, generators amd discriminator of the model
    ...
    def model_loss(self, input_real, input_z, output_dim, y, num_classes, label_mask, alpha=0.2, drop_rate=0.):
        """
    ...
    def discriminator(self,x, reuse=False, alpha=0.2, drop_rate=0., num_classes=10, size_mult=64):
        with tf.variable_scope ('discriminator', reuse=reuse):
            ...
            class_logits = tf.layers.dense (features, units=num_classes, name='discriminator_class_logits')
            ...
            mx = tf.reduce_max (class_logits, 1, keep_dims=True)
            stable_real_class_logits = class_logits - mx

            gan_logits = tf.log (tf.reduce_sum (tf.exp (stable_real_class_logits), 1)) + mx
